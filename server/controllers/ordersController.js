    ...
    async create(req, res) {
        ...
        try {
            ...
            if(isDeviceInDB.count === parseDevices.length) { 
                ...
                return ApiError.badRequest(res.json(`Нет доступа(${notFoundIdDevices.join(', ')}) в БД`));
            }

            return res.json("Спасибо за Ваш заказ, мы свяжимся с Вами позже");
        } catch (e) {
            return res.json(e);
        }
    }
            ...
            await Orders.findOne({where:{id}})
                .then( async data => {
                    if(data) {
                        ...
                        return res.json("Нет доступа в базу данных");
                    }
                })
        } catch (e) {
            return res.json("Обновленние не завершенно, не предвиденая ошибка: " + e);
        }
            ...
            await Orders.findOne({where:{id}})
                .then( async data => {
                    if(data) {
                        ...
                        return res.json("Нет доступа в БД");
                    }
                })
        } catch (e) {
            return res.json("Обновленние не завершенно, не предвиденая ошибка: " + e);
